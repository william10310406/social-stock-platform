...
# Stock Insight Platform - 修復版導出腳本
# 解決 SQL 輸出混入和格式問題
...
EXPORT_DIR="exports/stocks_fixed_${TIMESTAMP}"
DB_CONTAINER="stock-insight-db"
DATABASE="StockInsight"

echo "🔧 Stock Insight Platform - 修復版數據導出"
echo "=============================================="
...
docker exec "${DB_CONTAINER}" /opt/mssql-tools18/bin/sqlcmd \
    ...
    -s "," -W > stocks_clean.csv

# 清理 CSV（移除空行和 SQL 輸出）
grep -v "^$" stocks_clean.csv | grep -v "Changed database" | grep -v "rows affected" > stocks_data.csv
...
# 清理統計資料
grep -v "^$" stats_raw.csv | grep -v "Changed database" | grep -v "rows affected" > statistics.csv
...
docker exec "${DB_CONTAINER}" /opt/mssql-tools18/bin/sqlcmd \
    ...
    -Q "BACKUP DATABASE [${DATABASE}] TO DISK = '/var/opt/mssql/data/StockInsight_Fixed_Export.bak' 
        WITH FORMAT, INIT, NAME = 'StockInsight Fixed Export', SKIP, NOREWIND, NOUNLOAD;"

# 複製備份文件
docker cp "${DB_CONTAINER}:/var/opt/mssql/data/StockInsight_Fixed_Export.bak" ./
...
if [ -f "StockInsight_Fixed_Export.bak" ]; then
    FILE_SIZE=$(du -h StockInsight_Fixed_Export.bak | cut -f1)
    echo "✅ StockInsight_Fixed_Export.bak - 資料庫備份 (${FILE_SIZE})"
fi

echo ""
echo "🎯 修復完成！所有文件已清理並格式化正確。"
echo "使用方法："
echo "  - stocks_data.csv: 純淨的 CSV 數據，無 SQL 輸出"
echo "  - stocks_complete.json: 標準 JSON 格式，包含元數據和統計"
echo "  - StockInsight_Fixed_Export.bak: 完整資料庫備份"
