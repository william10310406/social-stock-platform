...
def send_friend_request(current_user):
    ...
    new_request = Friendship(requester_id=current_user.id, addressee_id=addressee_id, status='pending')
...
def get_pending_requests(current_user):
    ...
    pending_requests = Friendship.query.filter_by(addressee_id=current_user.id, status='pending').all()
