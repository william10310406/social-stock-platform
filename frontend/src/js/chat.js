...
const fetchConversations = async () => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            conversationsList.innerHTML = '<div class="p-4 text-center text-red-500">載入聊天列表失敗</div>';
        }
    } catch (error) {
...
const populateFriendSelect = (friends) => {
    friendSelect.innerHTML = '<option value="">選擇好友...</option>' +
...
const selectConversation = (conversationId, otherUser) => {
    ...
    document.querySelector(`[data-conversation-id="${conversationId}"]`).classList.add('bg-indigo-50');
